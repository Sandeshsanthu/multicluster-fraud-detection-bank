...
def compute_and_plot_global_shap(
    ...
    # --- FIX START ---
    # Handle SHAP output: some versions return [class0, class1], 
    # others return just the array for binary classification.
    if isinstance(shap_values, list):
        # Use class 1 (fraud) if it's a list
        shap_to_plot = shap_values[1]
    else:
        # Use the array directly if it's not a list
